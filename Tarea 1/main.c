...
int main(int argc, const char * argv[]) {
    ...
    const double dt = 0.01;               // Ancho temporal, en años
    ...
    // Alpha del potencial U(rho) = -alpha/rho (con transformación de unidades)
    const double alpha = (G*pow(SEC_YEAR, 2)*KG_EARTH/pow(MET_AU, 3))*m1*m2;
    ...
    printf("\t rho0 \t\t = \t%10.2f (AU)\n", rho0);
    printf("\t rhop0 \t\t = \t%10.2f (AU/años)\n", rhop0);
    printf("\t phi0 \t\t = \t%10.2f (rad)\n", phi0);
    printf("\t phip0 \t\t = \t%10.2f (rad/años)\n", phip0);
    ...
    double de, sgn;
    /* Definimos el signo del crecimiento de rho inicialmente como el signo de
       el valor inicial de rho prima */
    if (phip0 == 0) sgn = 1;
    else sgn = phip0 / fabs(phip0);

    // Comenzamos la iteración
    for (int i=0; i<nsteps-1; i++) {
        // Calculamos la diferencia entre Ueff y E
        de = E - U_eff(rho[i],l,mu,alpha);
        // Si la diferencia es <= a cero cambiamos de signo
        if (de <= 0 && i != 0) sgn *= -1;
        // Calculamos el paso siguiente para rho
        rho[i+1] = sgn*sqrt(2/mu *fabs(E - U_eff(rho[i],l,mu,alpha))) * dt + rho[i];
        // Calculamos el paso siguiente para phi a partir de rho
