...
/*  --- TAREA 2 (15%): ECUACIONES DIFERENCIALES CON VALORES INICIALES. --- */
...
const double p1_0 = 0.0;  // p1 es p_1, momento generalizado para la masa 1.
const double p2_0 = 6.5;  // p2 es p_2, momento generalizado para la masa 2.
...
int main(int argc, const char * argv[]) {
    ...
    const double dt = .001;   // paso temporal, división de la grilla de tiempo.
...
void eRK4(double* Yin, double* Yout, double dt) {
  ...
  F(Y1, _Y1);
  for (size_t i = 0; i < 4; i++) {  // Construcción de Y2[i] por eRK-4
    Y2[i] = dt/2 * _Y1[i] + Yin[i];
  }

  F(Y2, _Y2);
  for (size_t i = 0; i < 4; i++) {
    Y3[i] = dt/2 * _Y2[i] + Yin[i]; // Construcción de Y3[i] por eRK-4
  }

  F(Y3, _Y3);
  for (size_t i = 0; i < 4; i++) {
    Y4[i] = dt*_Y3[i] + Yin[i];     // Construcción de Y4[i] por eRK-4
  }
